...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `TinyHttp.swiftmodule/`
...
//      `import TinyHttp` above would fail at compile time.
//
//   2. The static archive `libTinyHttp.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "TinyHttp" }` and
//      `flattenPackage = "io.github.kotlinmania.tinyhttp"` configuration in
...
//      via the `TinyHttpLibrary` product.
//
...
final class TinyHttpExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "TinyHttp swift module imported cleanly")
    }
}
