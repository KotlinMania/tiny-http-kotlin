...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`tiny_http`](https://crates.io/crates/tiny_http).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `tiny_http` crate will be cloned into `tmp/` (gitignored) when porting begins.
...
Original work copyrighted by the upstream `tiny_http` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
